...
for (const shot of shots) {
  ...
  run([
    ...
    String(shot.fps),
    "-i",
    join(source, `${shot.id}-frame-%05d.jpg`),
    "-frames:v",
    String(shot.frames),
    "-c:v",
    "libx264",
...
run([
  ...shots.flatMap((shot) => ["-i", join(source, `${shot.id}.mp4`)]),
  "-i",
  ...
  join(destination, "bioscape-film.mp4"),
]);
writeFileSync(
  join(source, "film-manifest.json"),
  JSON.stringify(
    { duration, width: 1920, height: 1080, fps: 30, dissolve, shots },
    null,
    2,
  ),
);
console.log(`Saved ${join(destination, "bioscape-film.mp4")}`);
